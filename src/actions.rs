...
pub async fn end_playback(app: &mut App, session: &mut Session, error: Option<String>) {
    ...
    app.enter_search_mode(search_return_mode(app));
}
...
pub fn hide_selected(app: &mut App, now: std::time::Instant) {
    ...
    match app.hidden.add_video(&id, &title) {
        Ok(()) => {
            ...
            if !app.view_is_nested() && app.results.is_empty() {
...
mod tests {
    ...
    #[tokio::test]
    async fn the_playlist_thumbnails_are_fetched_for_the_playlist_videos() {
        let (tx, mut rx) = mpsc::unbounded_channel();
        let mut session = Session::default();
        let mut app = playlists_app(&tx, &mut rx, &mut session).await;
        open_playlist_with(&mut app, &tx, &mut session, StubYtDlp);
        app.set_results(vec![result("v0"), result("v1")], &playlist_target(&app));
    ...
    #[tokio::test]
    async fn end_playback_returns_to_the_playlist_it_started_from() {
        let (tx, mut rx) = mpsc::unbounded_channel();
        let mut session = Session::default();
        let mut app = playlists_app(&tx, &mut rx, &mut session).await;
        open_playlist_with(&mut app, &tx, &mut session, StubYtDlp);
        ...
        end_playback(&mut app, &mut session, None).await;
        assert_eq!(app.mode, Mode::Playlist, "開いていた中身へ戻る");
        assert!(app.playlist.is_some());
    }

    #[tokio::test]
    async fn end_playback_returns_to_the_playlists_list_that_is_open() {
        // 裏で再生しながら一覧を開いていられるので、終わっても一覧から動かさない。
        let (tx, mut rx) = mpsc::unbounded_channel();
        let mut session = Session::default();
        let mut app = playlists_app(&tx, &mut rx, &mut session).await;
        app.background = true;
        ...
        assert!(app.playlists.is_some());
    }

    #[tokio::test]
    async fn leaving_a_channel_opened_from_a_playlist_returns_to_the_playlist() {
        let (tx, mut rx) = mpsc::unbounded_channel();
        let mut session = Session::default();
        let mut app = playlists_app(&tx, &mut rx, &mut session).await;
        open_playlist_with(&mut app, &tx, &mut session, StubYtDlp);
        ...
        assert_eq!(app.mode, Mode::Playlist, "プレイリストの中へ戻る");
        assert!(app.playlist.is_some());
    }
    ...
    #[test]
    fn hiding_a_row_in_a_playlist_stays_in_the_playlist() {
        // 検索を通らずに一覧から入ってくるので、検索結果は空のままここへ来る。
        let (hidden, _path) = hidden_at("hide-in-playlist");
        let mut app = App {
            hidden,
            ..App::default()
        };
        ...
        app.playlist.as_mut().expect("playlist").state.results = vec![result("only")];
