...
fn new_parser(cols: u16, rows: u16) -> vt100::Parser {
    vt100::Parser::new(rows.saturating_add(1), cols, 0)
}

impl TextScreen {
    pub fn new(cols: u16, rows: u16) -> Self {
        Self {
            parser: new_parser(cols, rows),
    ...
    #[allow(dead_code, reason = "寸法は Geometry 側が持つので、確認用")]
    ...
    pub fn resize(&mut self, cols: u16, rows: u16) {
        if self.visible() == Rect::new(0, 0, cols, rows) {
            return;
        }
        self.parser = new_parser(cols, rows);
        self.frame = None;
    }
    ...
    pub fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        match &self.frame {
            ...
            None if !self.saw_frame => {
                let area = Rect {
                    height: area.height.min(visible.height),
                    ..area
                };
                render_screen(self.parser.screen(), area, buf);
            }
            None => {}
        }
    }
...
#[cfg(test)]
mod tests {
    use super::fixtures::{FRAME_END, TCT_PROLOGUE, TCT_RESTART};
    use super::*;

    fn render(bytes: &[u8], cols: u16, rows: u16, area: Rect) -> Buffer {
        let mut screen = TextScreen::new(cols, rows);
        screen.feed(bytes);
    ...
    fn frames_after_the_first_are_not_shifted_by_the_trailing_newline() {
        ...
        let mut screen = TextScreen::new(2, 3);
    ...
    fn size_reports_columns_then_rows() {
        assert_eq!(TextScreen::new(64, 20).size(), (64, 20));
    }
    ...
    fn resize_changes_the_grid_and_drops_the_stale_frame() {
        let mut screen = TextScreen::new(4, 2);
        screen.feed(&[TCT_PROLOGUE, b"\x1b[0;0fAAAA", FRAME_END].concat());
        screen.resize(8, 3);
    ...
    #[test]
    fn text_screen_ignores_kitty_leftovers() {
        // VO 切替の瞬間に前の kitty VO の後始末が入る (実測)。ST 無しの a=d も混じる。
        let leftovers = b"\x1b_Ga=d;\x1b\\\x1b_Ga=d;\x1b[?25h\x1b[?1003l\x1b[20;0f";
        let mut screen = TextScreen::new(4, 2);
        screen.feed(leftovers);
        assert!(screen.feed(&[TCT_PROLOGUE, b"\x1b[0;0fAB", FRAME_END].concat()));
        ...
        // 画像本体 (base64 相当の長い APC) もセルには入らない。
        let mut screen = TextScreen::new(4, 2);
        let mut big = b"\x1b_Ga=T,f=24,s=2,v=1,C=1,q=2,m=1;".to_vec();
        big.extend(std::iter::repeat_n(b'Q', 5_000));
