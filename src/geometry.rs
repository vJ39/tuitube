...
/// 端末に今のサイズを聞いて映像寸法を決める。
pub fn video_geometry() -> Geometry {
    let (cols, rows) = crossterm::terminal::size().unwrap_or((80, 24));
    geometry_for(cols, rows, cell_size())
}
...
pub fn geometry_for(cols: u16, rows: u16, cell: CellSize) -> Geometry {
    Geometry::new(
...
mod tests {
    ...
    fn geometry_matches_the_video_area_of_the_same_terminal_size() {
        let geometry = geometry_for(80, 24, CELL);
        assert_eq!(geometry.area, ui::video_area(Rect::new(0, 0, 80, 24)));
        assert_eq!(geometry.cell, CELL);
        // 映像領域 80x22 セル × 8x16 px。予算 640*360 に収まるので縮まない。
        assert_eq!(geometry.frame_px, (640, 352));
    }

    #[test]
    fn oversized_terminals_shrink_to_the_pixel_budget() {
        let geometry = geometry_for(200, 60, CELL);
    ...
    fn a_terminal_too_small_for_the_video_area_still_yields_a_frame() {
        let geometry = geometry_for(1, 1, CELL);
