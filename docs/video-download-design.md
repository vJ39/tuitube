...
- 失敗時: `set_error(Some("ダウンロードに失敗しました: <理由の1行>"))`。yt-dlpは半年以上更新していないとバージョン警告を必ず先頭に出すため、標準エラーをそのまま使うとステータス行(高さ1行)には警告だけが出て本当の理由が隠れる。`ERROR`を含む最初の行があればそれを使い、無ければ最後の空でない行を使う(`failure_reason`)
