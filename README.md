...
起動すると検索欄が開く。語句を入れて Enter で検索し、一覧で Enter を押すと再生が始まる。
...
| クリック | 検索欄ではカーソル移動、タブの行ではそのタブを選ぶ |
...
検索欄の `Ctrl+P` か結果一覧の `p` で、自分のプレイリストの一覧を開く(cookie 連携が要る)。`↑` `↓` で選び、`Enter` で中身を開き、`Esc` か `/` で戻る。

中身の画面の操作は結果一覧とほぼ同じで、タブの切り替え・「もっと見る」・`p`・クリックでの再生が無い。`h` はその動画だけを隠し、`Esc` か `/` で一覧へ戻る。URL で開いたプレイリストからは検索側へ戻る。中身は 50 件まで。
...
一覧のサムネイルと再生画面には、いいね済み・登録済みの印が出る。状態は OAuth を用意しているときに YouTube へ問い合わせて控え、`[engagement] ttl_secs`(既定 1 週間)ごとに取り直す。
